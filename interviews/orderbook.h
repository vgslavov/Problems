...
public:
    ...
    Order(size_t orderId, OrderType side, double price, int quantity)
    : d_orderId(orderId)
    , d_side(side)
    , d_price(price)
    , d_quantity(quantity)
    ...
    OrderType side() const { return d_side; }
...
class OrderBook {
public:
    using OrderType = Order::OrderType; // Type alias for backward compatibility
            ...
            int quantity)
    {
        ...
        d_orderToIndex[orderId] = d_orders.size() - 1; // Add to index map
        return true;
    }
